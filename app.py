...
def local_css():
    ...
    div.stButton > button {
        ...
        color: #1f2937 !important; /* Force dark text color for readability */
        text-align: left;
        transition: all 0.2s;
        font-weight: 600;
    }
    div.stButton > button:hover {
        ...
        color: #1f2937 !important; /* Ensure hover text remains readable */
...
if st.session_state.engine:
    ...
    with col_main:
        ...
        # Using placeholder text based on prompt
    ...
    with col_side:
        ...
        with m2:
            ...
            st.metric("Timer", f"{remaining}s", delta=None, delta_color="inverse")
        ...
        if current_node.type == 'terminal':
            ...
            for idx, choice in enumerate(current_node.choices):
                # Using key to ensure uniqueness
                if st.button(choice.text, key=f"btn_{current_node.id}_{idx}"):
