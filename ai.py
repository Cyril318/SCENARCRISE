...
class AIIntegration:
    ...
    def generate_image(self, prompt: str) -> Optional[str]:
        ...
        try:
            # Using the new SDK structure for image generation
            # Assuming 'imagen-3.0-generate-001' is the model to use.
            ...
            # response.generated_images is a list of objects containing image_bytes
            if response.generated_images:
                image_bytes = response.generated_images[0].image.image_bytes
                # Convert bytes to base64 data URI for Streamlit
                b64_img = base64.b64encode(image_bytes).decode('utf-8')
                return f"data:image/png;base64,{b64_img}"

        except Exception as e:
            print(f"Error generating image with Gemini: {e}")
            # Fallback to placeholder if API fails (e.g. model not available for key)
